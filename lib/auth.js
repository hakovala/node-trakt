			...
			this._requestAccess(poll.device_code)
				...
				.catch((err) => {
					if (err.response && err.response.statusCode === 400) {
						...
						debug('error: %d', err.response.statusCode);
						clearInterval(_int);
						return reject(err);
					}
				});
...
Auth.prototype.authUrl = function() {
	debug('generating authentication url');
...
Auth.prototype.export = function() {
	debug('export tokens: %o', this.tokens);
